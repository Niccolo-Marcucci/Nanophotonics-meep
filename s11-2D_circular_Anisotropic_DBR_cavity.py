...
class Simulation(mp.Simulation):
    ...
    def circular_deformed_cavity(self, pos):
        ...
        if r < D/2 : #or r > D/2 + N*period - (1-FF)*period:
            ...
            if is_groove:
                local_index = self.grating_index    + mod_tranches * (1-mpo.cos(theta)**8)  * (self.grating_index < self.background_index)
            else:
                local_index = self.background_index + mod_ridges   * (1-mpo.cos(theta)**8)  * (self.grating_index < self.background_index)
    ...
    def init_sources_and_monitors(self, f, df, source_pos, allow_profile=False) :
        ...
        if  allow_profile :
            self.field_profile = self.add_dft_fields([mp.Ey, mp.Ex], 1/np.array([.5924,.5903, .5812]),#f, 0, 1,
                                                     center = mp.Vector3(),
