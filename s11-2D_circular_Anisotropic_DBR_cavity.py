...
class Simulation(mp.Simulation):
    ...
    def circular_deformed_cavity(self, pos):
        ...
        if r < D/2 : #or r > D/2 + N*period - (1-FF)*period:
            ...
            if is_groove:
                local_index = self.grating_index    + mod_tranches * (mpo.cos(theta)**8)  * (self.grating_index < self.background_index)
            else:
                local_index = self.background_index + mod_ridges   * (mpo.cos(theta)**8)  * (self.grating_index < self.background_index)
