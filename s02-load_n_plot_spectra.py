...
hashtag ='55a7016478'#'5cab4c8862'#'e20d2ea866'#'62ef19ee4d'#'4dc3971d95'#'8a593f9138'#'2cb6bfb1fa'
...
for j, second_parameter in (enumerate(scnd_param['list'])):
    ...
    for k, file in enumerate(sim_filelist[j]) :
        data = mpo.loadmat(folder + '/' + file)
        Ex = data[f"field_profile_Ex_{k}"]
        Ey = data[f"field_profile_Ey_{k}"]
        data2save[f"field_{first_parameter[k]:.0f}"] = abs(Ex)**2 + abs(Ey**2)
        data2save[f"x_{first_parameter[k]:.0f}"] = data["field_profile_x"][0]
        data2save[f"y_{first_parameter[k]:.0f}"] = data["field_profile_y"][0]
    data2save["field_friequencies"] = first_parameter

    # plt.figure()
    # plt.imshow(images[k])

    io.savemat(folder + f'/sim_2D_{date}_{scnd_param["name"]}{second_parameter:.0f}_field.mat',data2save)
