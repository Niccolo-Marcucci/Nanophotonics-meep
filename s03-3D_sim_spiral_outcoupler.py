...
class Simulation():
    ...
    def init_sources_and_monitors(self, f, df, use_monitor=True) :
        ...
        self.monitors.append(self.sim.add_near2far(f, 0, 1, nearfield) if use_monitor else None)#, yee_grid=True))
...
Animate = mp.Animate2D(sim.sim, fields=mp.Hy, f=f, realtime=False, normalize=True,
                        output_plane=mp.Volume(center=center,size=mp.Vector3(0,simsize.y,simsize.z)))

sim.run(mp.at_every(0.1,Animate),until=20)
